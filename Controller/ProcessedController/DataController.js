import {unprocessedDbInstance, processedDbInstance, Predictions_DataDbInstance } from '../../Database/Database.js';
    ...
    try {
      ...
      for (const { code, isPlant, isPlanningPlant, isManufacturingPlant } of plants) {
        ...
        await processedDbInstance.run(query, [
          code, lat, lon, defaulted, finalIsPlant ? 1 : 0, finalIsPlanningPlant ? 1 : 0, finalIsManufacturingPlant ? 1 : 0
        ]);
                ...
                if (existing) {
                    ...
                    await processedDbInstance.run(
                        ...
                        [
                            ...
                            record.HubHeight, record.TowerHeight, record.TurbineClass, record.UnknownMaintPlant, record.UnknownPlanningPlant, 
...
export const uploadProcessedMaterialData = async (req, res) => {
    try {
            ...
            if (existing) {
                ...
                await processedDbInstance.run(
                    ...
                    [
                        ...
                        record.Serial_No_Profile, record.ReplacementPart, record.UsedInSBom, record.ViolationReplacementPart, 
                        record.MaterialCategory, record.UnknownPlant,
                        record.Material, record.Plant,
                    ]
                );
            } else {
                ...
                await processedDbInstance.run(
                    ...
                    [
                        ...
                        record.BatchManagementPlant, record.Serial_No_Profile, record.ReplacementPart, 
                        record.UsedInSBom, record.ViolationReplacementPart, record.MaterialCategory, record.UnknownPlant
                    ]
                );
            }
        }
        ...
        await processedDbInstance.run('ROLLBACK');
        console.error('Error saving cleaned MaterialData:', error);
        res.status(500).json({ success: false, message: 'Failed to save cleaned MaterialData', error: error.message });
    }
};
...
export const getProcessedMaterialData
= async (req, res) => {
    try {
...
export const getProcessedTurbineData
= async (req, res) => {
    try {
