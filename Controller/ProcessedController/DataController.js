...
export const uploadPredictionsData = async (req, res) => {
    try {
        ...
        for (const record of processed_Data) {
            await Predictions_DataDbInstance.run(
                ...
                [
                    ...
                    record.TurbineLatitude ? parseFloat(record.TurbineLatitude) : null, // Ensure float conversion
                    record.TurbineLongitude ? parseFloat(record.TurbineLongitude) : null // Ensure float conversion
                ]
            );
        }
        ...
        res.status(201).json({ success: true, message: 'Processed data successfully persisted' });
    } catch (error) {
        await Predictions_DataDbInstance.run('ROLLBACK');
        console.error('Error saving Processed data:', error);
        res.status(500).json({ success: false, message: 'Failed to save Processed data', error: error.message });
    }
};
...
= async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch processed data', error: error.message });
    }
}
