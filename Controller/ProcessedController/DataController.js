import {
  ...
  try {
    geoMapping = JSON.parse(
      await fs.readFile(new URL('../../utils/geo_mapping.json', import.meta.url))
    );
  } catch (err) {
  ...
  export const syncPlantCoordinates = async (req, res) => {
    ...
    try {
      ...
      for (const { code, isPlant, isPlanningPlant, isManufacturingPlant } of plants) {
      ...
      res.status(200).json({ success: true, message: '✅ Plant table updated with roles & coordinates' });
    } catch (error) {
      ...
      res.status(500).json({ success: false, message: 'Failed to sync plant roles', error: error.message });
    }
  };
  ...
  export const uploadProcessedTurbineData = async (req, res) => {
    try {
      ...
      for (const record of cleanedData) {
        ...
        const exists = await processedDbInstance.get(
          `SELECT 1 FROM TurbineData WHERE FunctionalLoc = ?`,
          [record.FunctionalLoc]
        );
  
        const values = [
          record.Description, record.MaintPlant, record.PlanningPlant, record.Platform,
          record.WTShortName, record.TurbineModel, record.MkVersion, record.Revision,
          record.NominalPower, record.OriginalEqManufact, record.SBOMForTurbine,
          record.SCADAName, record.SCADAParkID, record.SCADACode, record.SCADAFunctionalLoc,
          record.TechID, record.Region, record.Technology, record.HubHeight, record.TowerHeight,
          record.TurbineClass, record.UnknownMaintPlant, record.UnknownPlanningPlant,
          record.TurbineLatitude ? Number(parseFloat(record.TurbineLatitude).toFixed(6)) : null,
          record.TurbineLongitude ? Number(parseFloat(record.TurbineLongitude).toFixed(6)) : null
        ];
  
        if (exists) {
          await processedDbInstance.run(
            `UPDATE TurbineData SET 
              Description = ?, MaintPlant = ?, PlanningPlant = ?, Platform = ?, WTShortName = ?, TurbineModel = ?, 
              MkVersion = ?, Revision = ?, NominalPower = ?, OriginalEqManufact = ?, SBOMForTurbine = ?, 
              SCADAName = ?, SCADAParkID = ?, SCADACode = ?, SCADAFunctionalLoc = ?, TechID = ?, Region = ?, 
              Technology = ?, HubHeight = ?, TowerHeight = ?, TurbineClass = ?, UnknownMaintPlant = ?, UnknownPlanningPlant = ?, 
              TurbineLatitude = ?, TurbineLongitude = ? 
             WHERE FunctionalLoc = ?`,
            [...values, record.FunctionalLoc]
          );
        } else {
          await processedDbInstance.run(
            `INSERT INTO TurbineData (
              ...
              UnknownMaintPlant, UnknownPlanningPlant, TurbineLatitude, TurbineLongitude
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [record.FunctionalLoc, ...values]
          );
        }
      }
  ...
  export const uploadProcessedMaterialData = async (req, res) => {
    try {
      ...
      for (const record of cleanedData) {
        ...
        const values = [
          record.Description, record.PlantSpecificMaterialStatus, record.BatchManagementPlant,
          record.Serial_No_Profile, record.ReplacementPart, record.UsedInSBom || null,
          record.ViolationReplacementPart || null, record.MaterialCategory || null, record.UnknownPlant || null
        ];
  
        if (exists) {
          await processedDbInstance.run(
            `UPDATE MaterialData SET 
              Description = ?, PlantSpecificMaterialStatus = ?, BatchManagementPlant = ?, 
              Serial_No_Profile = ?, ReplacementPart = ?, UsedInSBom = ?, 
              ViolationReplacementPart = ?, MaterialCategory = ?, UnknownPlant = ?
             WHERE Material = ? AND Plant = ?`,
            [...values, record.Material, record.Plant]
          );
        } else {
          await processedDbInstance.run(
            ...
            [record.Material, record.Plant, ...values]
          );
        }
      }
      ...
      await processedDbInstance.run('COMMIT');
      await Predictions_DataDbInstance.run('COMMIT');
      res.status(201).json({ success: true, message: 'Processed material data successfully persisted' });
    } catch (error) {
      await processedDbInstance.run('ROLLBACK');
      await Predictions_DataDbInstance.run('ROLLBACK');
      console.error('Error saving processed material data:', error);
      res.status(500).json({ success: false, message: 'Failed to save processed material data', error: error.message });
    }
  };
  ...
  export const uploadTurbinePredictionsData = async (req, res) => {
    try {
      ...
      for (const record of processed_Data) {
        const exists = await Predictions_DataDbInstance.get(
          `SELECT 1 FROM TurbineData WHERE FunctionalLoc = ?`,
          [record.FunctionalLoc]
        );
  
        const values = [
          record.Description, record.MaintPlant || null, record.PlanningPlant || null,
          record.Platform || null, record.WTShortName || null, record.TurbineModel || null,
          record.MkVersion || null, record.Revision || null, record.NominalPower || null,
          record.OriginalEqManufact || null, record.SBOMForTurbine || null, record.SCADAName || null,
          record.SCADAParkID || null, record.SCADACode || null, record.SCADAFunctionalLoc || null,
          record.TechID || null, record.Region || null, record.Technology || null, record.HubHeight || null,
          record.TowerHeight || null, record.TurbineClass || null, record.UnknownMaintPlant || null,
        ...
        if (exists) {
          await Predictions_DataDbInstance.run(
            `UPDATE TurbineData SET 
              Description = ?, MaintPlant = ?, PlanningPlant = ?, Platform = ?, WTShortName = ?, 
      ...
      await Predictions_DataDbInstance.run('COMMIT');
      res.status(201).json({ success: true, message: 'Processed turbine data successfully persisted' });
    } catch (error) {
      await Predictions_DataDbInstance.run('ROLLBACK');
      console.error('Error saving processed turbine data:', error);
      res.status(500).json({ success: false, message: 'Failed to save processed turbine data', error: error.message });
    }
  };
  ...
  export const getProcessedMaterialData = async (req, res) => {
    try {
      const data = await processedDbInstance.all('SELECT * FROM MaterialData');
      res.status(200).json({ success: true, data });
    } catch (error) {
      console.error('Error fetching processed MaterialData:', error);
      res.status(500).json({ success: false, message: 'Failed to fetch processed MaterialData', error: error.message });
    }
  };
  ...
  export const getProcessedTurbineData = async (req, res) => {
    try {
      ...
      res.status(500).json({ success: false, message: 'Failed to fetch processed TurbineData', error: error.message });
    }
  };
